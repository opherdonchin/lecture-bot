...
A conforming specification must contain four top-level sections, in this order, with these names:
...
- **A "Delegated to runtime" section** (see §7).
...
- **B3. Interaction repertoire**, with two required and two recommended internal subsections:
  - **B3.1. Core decision architecture** (required)
  - **B3.2. Interaction modes** (required)
  - **B3.3. Applied interactional guidance** (recommended)
  - **B3.4. Interaction lifecycle** (recommended)

B3.1 and B3.2 may not be merged with each other or with any other subsection. B3.3 and B3.4, when present, must remain distinct from each other and from B3.1 and B3.2.

### 4.2 View of the subject matter (recommended)
...
B2, B3.1, and B3.2 each establish a structural commitment that shapes the tutor's behavior. For each of these subsections, the specification must either:
...
- **B2** — a named set of attention dimensions along which the tutor tracks the student and the interaction. Each named dimension must have a short operational gloss.
- **B3.1** — an organizing judgment the tutor runs on each turn. This may be a decision rule, a central question, or an explicit binary. It must be stated as the first substantive content of B3.1 so it can be cleanly identified.
- **B3.2** — a named set of recurring interaction modes. Each named mode must have a short operational gloss.
...
- **C1. Evaluation structure** — the role evaluation plays in the interaction and how it relates to teaching.
- **C2. Evaluation criteria** — what counts as stronger or weaker evidence of understanding. C2 is required unless C1 declares that evaluation has no role in this tutor's behavior.

### 5.2 Evaluation shape declaration (required)

C must declare the **evaluation shape**, meaning the structural form of evaluative state the tutor will maintain. The specification must choose one of:

- **Defined in specification**: C defines a schema for evaluative state (for example, a mastery scale, evidence-field structure, or equivalent), which the generator passes through unchanged.
- **Delegated to runtime**: C explicitly states that schemas for evaluative state are left to the runtime prompt, and the generator is responsible for defining them.
- **No evaluative role**: C1 declares the tutor does not maintain evaluative state, in which case no shape is needed.

Silence is not permitted.

### 5.3 Evidence-quality guidance (required when C2 is present)

C2 must describe, qualitatively or structurally, what counts as stronger evidence of understanding and what counts as weaker evidence. This guidance governs how conservatively the tutor updates evaluative state at runtime.

---

## 6. Section D — Success condition

Section D states what a successful turn and a successful overall interaction are trying to achieve.
...
- **evaluative state schemas**: defined in specification, delegated to runtime, or not applicable (per §5.2).
...
The specification describes pedagogy, identity, and priorities. It does not describe JSON shapes, input variable names, state update rules, or output formatting unless overriding a runtime default under §7.

### 8.2 Explicit over implicit

Where the specification makes a commitment, it should make it explicit enough that the generator does not have to reconstruct it from mood. Named taxonomies should be named. Priority orderings should be stated as orderings. Decision rules should be stated as decision rules.

### 8.3 Consistency across sections
...
### 8.4 No generator-targeted instructions
...
## 9. Generator behavior toward recommended sections
...
## 10. What this contract does not require
...
- [ ] Top-level sections A, B, C, D are present, named, and in order.
- [ ] A contains A1, A2, A3.
...
- [ ] The specification is internally consistent per §8.3.
