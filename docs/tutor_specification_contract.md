...
B2 shapes the tutor's behavior toward depth, breadth, distinctions, and integration. When present, B2 governs those aspects of the runtime prompt. When absent, the generator may infer defaults from C, D, E, and the tutor's domain.
...
- **C5. Recorded decision logic** (required)
...
C1, C2, and C5 may not be merged with each other or with any other subsection. C3 and C4, when present, must remain distinct from each other and from C1, C2, and C5.
...
Other explicitly named decision-process forms are also allowed, but they are optional rather than default. If one is used, it must be described with comparable structural detail. At minimum, the specification must state what is being decided, what the available options or outputs are, what considerations or evidence bear on the choice, and how the choice affects the tutor's behavior.
...
These C1 forms are pedagogical behavior specifications. They do **not** by themselves define a runtime output shape, require exposure of private reasoning, or fully determine the recorded decision-logic schema. Recorded decision logic is governed by C5 and by the runtime contract.
...
### 5.5 C4 Applied interactional guidance (recommended)
...
### 5.6 C5 Recorded decision logic (required)

C5 must define how the tutor's cognition is recorded in inspectable form.

At minimum, C5 must state:

- that the tutor has a recorded decision-logic artifact associated with each turn,
- whether that artifact is always present,
- whether it may be blank, and
- what counts as the blank form.

For the purposes of this contract, **blank** means **present but empty**. If the specification permits blank recorded decision logic, it must name the blank form explicitly. The default blank form is `{}` unless the specification states otherwise.

C5 must also describe the tutor-specific structure of recorded decision logic when it is non-blank.

The shape may be defined as:

- a named set of fields,
- an ordered sequence of named entries,
- a named nested structure,
- or another explicitly described JSON-serializable form.

Whatever form is chosen, C5 must specify with comparable clarity:

- the major components or fields,
- the role of each component,
- the expected granularity of each component, and
- any conditionality that affects when components appear or may be blank.

If the recorded decision logic shape varies by turn condition, decision-process branch, or tutor mode, C5 must state that variation explicitly.

C5 may define a very small shape. It does not need to mirror all latent tutor cognition. But it must be specific enough that a generator can preserve it and a reviewer can inspect it.

C5 is part of tutor cognition, not a separate pedagogical system. It records the tutor's decision-making in inspectable form.

The recorded decision logic in C5 must be consistent with the decision process defined in C1. It does not need to expose every latent consideration, but it must not systematically omit or contradict the main decisions that C1 says the tutor makes.

If C1 defines a documented decision algorithm, C5 should normally preserve its main decision-relevant steps in recorded form, though not necessarily at the same level of detail.

C5 may also state the intended purpose of recorded decision logic, such as debugging, inspectability, auditing, or tutor-quality review.
...
- **recorded decision-logic transport, storage, and visibility**: by default delegated to the runtime contract unless the specification narrows or constrains them.
...
The specification describes pedagogy, identity, priorities, recorded decision-logic structure, and evaluation commitments. It does not describe low-level JSON transport, input variable names, state update rules, or output formatting unless overriding a runtime default under §8.

### 9.2 Explicit over implicit

Where the specification makes a commitment, it should make it explicit enough that the generator does not have to reconstruct it from mood. Named taxonomies should be named. Priority orderings should be stated as orderings. Decision processes should be stated in their chosen form. Recorded decision-logic shapes should be stated as inspectable structures.
...
- recorded decision logic in C5 must be consistent with the decision process in C1 and must not silently contradict the tutor's priority ordering;
- documented decision processes in C1, when present, must be consistent with the stated interaction modes in C2.
...
- the specific recorded decision-logic schema, beyond the structural requirements in §5.6,
...
- [ ] C5 states the presence requirement for recorded decision logic and names the blank form if blank logic is permitted.
- [ ] C5 defines the non-blank recorded decision-logic shape with sufficient inspectable structure.
...
- [ ] C5 explains the intended use of recorded decision logic.
...
- [ ] C5 distinguishes different recorded decision-logic shapes for different turn conditions.
