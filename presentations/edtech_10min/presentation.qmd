...
<div><strong>4%</strong><span>Fourth</span></div>
<div><strong>3%</strong><span>Fifth</span></div>
</div>
</div>
</div>
:::
...
This is implemented behavior, not just a design intention. `app.bot_engine.compute_weighted_grade()` uses `[55, 25, 13, 4, 3]`, and `/get_grade` and `/generate_report` do not ask the model to compute the final numeric grade.
